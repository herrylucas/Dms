...
$(document).ready(function () {
	...
	$(".notes").click(function(e) {
		e.preventDefault();
		$("#newitem").load("ajax/divs/newdrivernote.php", {did: $(this).attr("id").replace("notes_","")}, function () {
			$(this).fadeIn().draggable();
		})
	})
...
<tr>
	...
	<th db="demploymentdate">Employment Date</th>
	<th db="dcontractexpiry">Contract Expiry</th>
	<th>Status</th>
	<th></th>
</tr>
<?php
$res = pg_query($con, "select *, to_char(dlicenseexp, 'DD/MM/YYYY') as dlicenseexp, to_char(dpassportexp, 'DD/MM/YYYY') as dpassportexp,
to_char(demploymentdate, 'DD/MM/YYYY') as demploymentdate, to_char(dcontractexpiry, 'DD/MM/YYYY') as dcontractexpiry,
...
case when dcontractexpiry <= now() + interval '30 days' then true else false end as contractexpiring
...
while($row = pg_fetch_assoc($res)) {
	...
	echo "
		<tr class='tbl'>
				...
				<td>" . uinput("demploymentdate", $row["did"], $row["demploymentdate"], "drivers", "demploymentdate", "did", $row["did"], false,false,80,"dp centered demploymentdate",null,null,true,true,false) . "</td>
				<td>" . uinput("dcontractexpiry", $row["did"], $row["dcontractexpiry"], "drivers", "dcontractexpiry", "did", $row["did"], false,false,80,"dp centered" . $ceclass,null,null,true,true,false) . "</td>
				<td>" . $status . "</td>
				<td class='delbtn'><button title='ADD NOTES' class='notes' id='notes_" . $row["did"] . "'><img class='smallbutton' src='icons/editor.png'></button></td>
